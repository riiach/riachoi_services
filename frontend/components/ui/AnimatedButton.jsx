...
const AnimatedButton = ({ type = "button", text, href, w }) => {
    if (href) {
        return (
            <Link
                href={href}
                className={`${w === "w-full" ? "w-full" : "w-fit"} group bg-accent relative h-fit cursor-pointer overflow-hidden rounded-full px-6 py-2 text-white`}
            >
